import Configuration, { ItemsEnum, HashOptions } from "./configuration.js";
import Orders from "./order.js";
//Potentially External
import Toast from "./toast.js";
import fs from "node:fs";
//Types
import { MultiOrderReturn } from "./type_orders.js";
import { TaxRates, ServiceCharge } from "./type_configuration.js";
...
/**
 * Converts Toast Data to DataPlus Journal
 * @param property{string} - GUID from Toast 000-00000-00000-0000
 * @param businessDate{string} - yyyymmdd
 */
async function DPJournal(api: Toast, property: string, businessDate: string) {
  //Initialize API
  try {
  ...
  for (const order of orders.slice(0, 3)) {
    ...
    const mealPeriod = settings.restaurantServices.get(
      order.restaurantService.guid
    );
    ...
    if (order.checks.filter((x) => x.voided).length != order.checks.length) {
      const guestIndex = covers.findIndex(
        (x) => x.revenueCenter == revenueCenter && x.mealPeriod == mealPeriod
      );
      if (guestIndex != -1) {
    ...
    for (const check of order.checks) {
      ...
      for (const item of check.selections) {
        ...
        for (const modifier of item.modifiers) {
          ...
          const salesCategory = settings.salesCategories.get(
